...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE constraint_name = 'employee_sessions_employee_id_fkey'
    ) THEN
        ALTER TABLE employee_sessions 
        ADD CONSTRAINT employee_sessions_employee_id_fkey 
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
        ...
        ALTER TABLE sales_history ADD COLUMN member_info TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'sales_history' AND column_name = 'coupon_info') THEN
        ALTER TABLE sales_history ADD COLUMN coupon_info TEXT;
    END IF;
END $$;
